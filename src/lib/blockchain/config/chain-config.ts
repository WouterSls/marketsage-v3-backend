...
export function getChainConfig(chainId: bigint): ChainConfig {
  try {
    const chainConfig = Object.values(chainConfigs).find((config) => config.id === chainId);
    if (!chainConfig) {
      throw new TechnicalError(`Unsupported chainId: ${chainId}`);
    }

    return chainConfig;
  } catch (error) {
    console.error("Error getting chain configuration: ", error);
    throw new TechnicalError(`Error getting chain configuration`);
  }
}
