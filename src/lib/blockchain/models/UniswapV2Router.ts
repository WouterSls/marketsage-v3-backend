...
import { calculateSlippageAmount, extractRawTokenOutputFromLogs } from "../utils/blockchain-utils";
...
export class UniswapV2Router {
  ...
  constructor(
    ...
    this.routerContract = new ethers.Contract(this.routerAddress, UNISWAP_V2_ROUTER_ABI, this.wallet);
  ...
  async simulateBuySwap(token: ERC20, usdValue: number): Promise<void> {
    try {
      ...
      throw new RouterError(`Transaction simulation failed: ${errorMessage}`);
    }
  }
  async swapEthInUsdForToken(token: ERC20, usdValue: number): Promise<TradeSuccessInfo> {
    try {
      ...
      try {
        ...
        if (!txReceipt) throw new TechnicalError("Transaction receipt not found");

        const transactionStatus = txReceipt.status;
        if (transactionStatus !== 1) {
          throw new TechnicalError(
            "Transaction failed: execution reverted, ran out of gas, or encountered another error",
          );
        }
  ...
  private async estimateUsdValueToTokenTrade(
    ...
    try {
      ...
      if (error instanceof TechnicalError) throw error;
      const errorMessage = error instanceof Error ? error.message : "An unknown error occurred";
      throw new TechnicalError(`UniV2Router buy output error: ${errorMessage}`);
    }
  }
  ...
  async simulateSellSwap(erc20: ERC20, rawSellAmount: bigint): Promise<void> {
    try {
      ...
      if (rawBalance <= 0n) {
        throw new RouterError(`No token balance for: ${erc20.getName()}`);
      }

      if (allowedBalance <= 0n) {
        throw new RouterError(`No allowance for token: ${erc20.getName()}`);
      }

      if (allowedBalance < rawSellAmount) {
        throw new RouterError(
          `Insufficient allowance for token: ${erc20.getName()} | allowed: ${ethers.formatUnits(allowedBalance, erc20.getDecimals())} - sell amount: ${ethers.formatUnits(rawSellAmount, erc20.getDecimals())}`,
        );
      }
      ...
      const errorMessage = error.toString().includes("reverted") ? error.toString() : "Unknown simulation error";
      throw new TechnicalError(`Transaction simulation failed: ${errorMessage}`);
    }
  }
  async swapExactTokenForEth(token: ERC20, rawSellAmount: bigint): Promise<TradeSuccessInfo> {
    try {
      ...
      if (rawBalance === 0n) {
        throw new RouterError(`No balance for token: ${token.getName()}`);
      }

      if (allowedBalance === 0n) {
        throw new RouterError(`No allowance for token: ${token.getName()}`);
      }

      if (allowedBalance < rawSellAmount) {
        throw new RouterError(
          `Insufficient allowance for token: ${token.getName()} | allowed: ${ethers.formatUnits(allowedBalance, token.getDecimals())} - sell amount: ${ethers.formatUnits(rawSellAmount, token.getDecimals())}`,
        );
      }

      if (rawSellAmount <= 0n) {
        throw new RouterError(`Insufficient sell amount: ${ethers.formatUnits(rawSellAmount, token.getDecimals())}`);
      }

      if (rawSellAmount > rawBalance) {
        throw new RouterError(
          `Insufficient balance for token: ${token.getName()} | sell amount: ${ethers.formatUnits(rawSellAmount, token.getDecimals())} - balance: ${ethers.formatUnits(rawBalance, token.getDecimals())}`,
        );
      }
      ...
      if (!txReceipt) throw new RouterError("Transaction receipt not found");

      const transactionStatus = txReceipt.status;
      if (transactionStatus !== 1) {
        throw new RouterError("Transaction failed: execution reverted, ran out of gas, or encountered another error");
      }
      ...
      const errorMessage = error.toString().includes("reverted") ? error.toString() : "Unknown simulation error";
      throw new TechnicalError(`Transaction failed: ${errorMessage}`);
    }
  }
  private async estimateTokenToEthTrade(
    ...
    try {
      ...
      if (error instanceof RouterError) throw error;
      const errorMessage = error instanceof Error ? error.message : "An unknown error occurred";
      throw new RouterError(`UniV2Router sell estimation error: ${errorMessage}`);
    }
  }
}
